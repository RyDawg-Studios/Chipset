...
class DefaultEnemy(ShooterEnemy):
    ...
    def die(self, killer):
        ...
        if random.randint(1,1) == 1:
            wd = WeaponData(weaponClass=Medpack, upgrades=[])
            self.man.add_object(obj=PickupWeapon(man=self.man, pde=self.pde, position=list(self.rect.center), rotation=random.randint(0,360), weaponData=wd, speed=[4, 4]))