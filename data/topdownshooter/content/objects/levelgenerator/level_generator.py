...
class LevelGenerator(Actor):
    ...
    def generate_enemy_spawnpoints(self):
        for i in range(self.generate_enemy_count(complexity=self.complexity)):
            ...
            n = self.man.add_object(DefaultEnemy(man=self.man, pde=self.pde, position=self.get_spawnpoint(), hp=100*self.complexity**(1/8)))
            n.weapon = self.man.add_object(self.weaponladder[w](man=self.man, pde=self.pde, owner=n, position=n.position))
